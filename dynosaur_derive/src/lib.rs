...
fn mk_box_blanket_impl(item_trait: &ItemTrait) -> TokenStream {
    ...
    if self_receiver.shared_ref
        && !self_receiver.owned
        && !self_receiver.mut_ref
        && !self_receiver.box_ref
    {
        let items = items.clone();

        result.extend(
            quote! {
                impl #blanket_impl_generics #item_trait_ident #trait_generics for & #blanket #blanket_where_clause where DYNOSAUR: ?Sized {
                    #(#items)*
                }
            }
        );
    ...
    if (self_receiver.shared_ref || self_receiver.mut_ref)
        && !self_receiver.owned
        && !self_receiver.box_ref
    {
        let items = items.clone();
        ...
        result.extend(
            quote! {
                impl #blanket_impl_generics #item_trait_ident #trait_generics for &mut #blanket #blanket_where_clause where DYNOSAUR: ?Sized {
                    #(#items)*
                }
            }
        );
    ...
    if self_receiver.owned
        || self_receiver.shared_ref
        || self_receiver.mut_ref
        || self_receiver.box_ref
    {
        result.extend(
